...
def exam_compile_code(request):
    """
    View for exam compiler - uses AJAX to compile code without page reload
    """
    if request.method == "POST":
        ...
        # For debugging
        print(f"Received exam compiler request: language={language}, code length={len(code)}")
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            # Return the output fragment that matches what the JS is expecting
            return render(request, 'compile_result_ajax.html', {
                'output': output
            })
        else:
            # Fallback for non-AJAX requests
            return render(request, 'exam_compiler.html', {
                'output': output,
                'code': code,
                'language': language
            })
    ...
    # Default render for GET requests
    return render(request, 'exam_compiler.html', {'language': 'java'})
